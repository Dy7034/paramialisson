// ============================================
// NUESTRO PEQUEÑO MUNDO - LÓGICA v4.0
...
const CONFIG = {
    ...
    wishes: [
        'Deseo que seas feliz siempre, mi niña 💖',
        'Deseo cumplir cada uno de tus sueños ✨',
        'Deseo verte sonreír todos los días 😊',
        'Deseo ser tu compañero de vida para siempre 💑',
        'Deseo que este amor nunca termine 💕',
        'Deseo ser tu primer paciente cuando seas odontóloga 🦷',
        'Deseo que cumplas 18 años rodeada de amor 🎂',
        'Deseo construir un hogar contigo 🏡',
        'Deseo que Kuromi te proteja siempre 🎀'
    ],
...
// REASONS
function initReasons() { const c = document.getElementById('reasonsContainer'); CONFIG.reasons.forEach((r, i) => { const card = document.createElement('div'); card.className = 'reason-card'; card.innerHTML = `<div class="reason-number">${i + 1}</div><div class="reason-text">${r}</div>`; c.appendChild(card); }); }
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                setTimeout(() => entry.target.classList.add('revealed'), Array.from(promesas).indexOf(entry.target) * 300);
            }
        });
    }, { threshold: 0.3 });
    promesas.forEach(p => observer.observe(p));
    promesas.forEach(p => p.addEventListener('click', () => p.classList.add('revealed')));
}

// 🎮 JUEGO DE KUROMI - RECOLECTAR ROSAS
...
function endGame() {
    ...
    if (gameScore > highScore) {
        highScore = gameScore;
        localStorage.setItem('kuromiHighScore', highScore);
        document.getElementById('highScore').textContent = highScore;
    }
    
    let message = '';
    if (gameScore >= 30) {
        message = '¡Increíble, Alisson! 🌟 Eres tan dedicada como nuestro amor. ¡Kuromi está orgullosa de ti!';
    } else if (gameScore >= 20) {
        message = '¡Muy bien, mi niña! 💖 Tienes muy buena puntería, como cuando me robas el corazón.';
    } else if (gameScore >= 10) {
        message = 'Nada mal 😊 Pero aún hay más amor por recolectar, como en nuestra relación.';
    } else {
        message = 'Vuelve a intentarlo, Chuky 💕 Cada rosa que atrapaste es un beso que te debo.';
    }
    
    document.getElementById('finalScore').textContent = gameScore;
    document.getElementById('gameMessageText').textContent = message;
    document.getElementById('gameOverlay').classList.add('active');
    document.getElementById('gameCanvas').classList.add('hidden');
    document.getElementById('gameStartBtn').style.display = 'block';
    document.getElementById('gameStartBtn').textContent = '🎮 Jugar de Nuevo';
}
...
function initAllSections() {
    ...
    document.querySelectorAll('.section-title, .section-subtitle, .counter-container, .heart-container, .gallery-item, .dream-item, .reason-card, .birthday-special, .sealed-letter, .promesas-container').forEach(el => { el.classList.add('reveal'); revealObserver.observe(el); });
